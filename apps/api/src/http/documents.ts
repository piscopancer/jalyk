...
const mapDocErrors = <A, R>(
  eff: Effect.Effect<A, NotFoundError | DbError, R>,
) =>
  eff.pipe(
    ...
    handlers
      .handle('create', ({ path, payload }) =>
        writeAccess(path.projectId).pipe(
          Effect.andThen(() =>
            mapDocErrors(
              createDocument(path.projectId, payload.type, payload.draft ?? {}),
            ),
          ),
          Effect.tap((doc) =>
      ...
      .handle('list', ({ path, urlParams }) =>
        access(path.projectId).pipe(
          Effect.andThen(() =>
            mapDocErrors(listDocuments(path.projectId, urlParams.type)),
          ),
      ...
      .handle('counts', ({ path }) =>
        access(path.projectId).pipe(
          Effect.andThen(() =>
            mapDocErrors(countDocumentsByType(path.projectId)),
          ),
        ),
      )
      .handle('get', ({ path }) =>
        access(path.projectId).pipe(
          Effect.andThen(() =>
            mapDocErrors(getDocument(path.projectId, path.id)),
          ),
          Effect.flatMap((doc) =>
            doc ? Effect.succeed(toInfo(doc)) : new NotFound(),
          ),
        ),
      )
      ...
      .handle('publish', ({ path }) =>
        writeAccess(path.projectId).pipe(
          Effect.andThen(() =>
            mapDocErrors(publishDocument(path.projectId, path.id)),
          ),
          Effect.tap((doc) =>
            publishEvent(path.projectId, {
              kind: 'publish',
      ...
      .handle('resetDraft', ({ path }) =>
        writeAccess(path.projectId).pipe(
          Effect.andThen(() =>
            mapDocErrors(resetDraftDocument(path.projectId, path.id)),
          ),
          Effect.tap((doc) =>
            publishEvent(path.projectId, {
              kind: 'reset',
      ...
      .handle('delete', ({ path }) =>
        writeAccess(path.projectId).pipe(
          Effect.andThen(() =>
            mapDocErrors(deleteDocument(path.projectId, path.id)),
          ),
          Effect.tap((deleted) =>
            publishEvent(path.projectId, {
              kind: 'delete',
      ...
      .handle('putSchema', ({ path, payload }) =>
        writeAccess(path.projectId).pipe(
          Effect.andThen(() =>
            mapDocErrors(putSchemaSnapshot(path.projectId, payload.snapshot)),
          ),
