...
1. передать вебсокет сервер в контексте трпц и просить его прямо внутри мутации
2. перенести эксперсс сервер в пекедж трпц и связать их там, затем вызывать запуск сервера в приложении сервера
...
! просматривать json значение в тулбаре каждого поля, так можно удобно просмотреть контент типтап едитора. использовать shiki.  
...
<!-- ! ПОЧЕМУ ТИПА ПОЛЯЯ JSON УПИРАЕТСЯ В БЕСКОНЕЧНЫЙ ЦИКЛ? УБРАТЬ НА ANY -->

! если поле было типа JSON строки, но затем его изменили в студии на объект, указав другую zod схему, то валидация вернет ошибку, которую нужно показать вместе с полем и предложить удалить и скопировать текущие "неверные" данные (строку). ! любое текстовое поле можно открыть на весь экран ! для поля можно указать иконку и оно будет рендериться слева от названия ! если нужно рендерить поле самостоятельно, нужно просто указать его id и id документа, так документ по айди будет найден в бд, прочитан его тип, по типу будет найдена схема документа а внутри схемы по пути поля будет найдена другая инфа нужная для поля типа shape
...
! форму документа можно открыть на весь экран, когда та рендерится для ссылки в другом документе. Потому что форма может быть широкой, нужно обернуть ее в контейнер, и компоненты внутри ! если массив содержит только документы одного типа - есть кнопка рендерить как таблицу. поле станет столбиком, который можно свернуть - оно останется, но скроются значения и будет показана иконка поля.

## Preview

! справа как истории в тг показывает людей, которые сидят в поле документа ! превью документа с повторяющимся фоном стрелка вправо чтобы показать движение вправо ! добавить контекст из base ui, который будет работать как доп меню
...
! default structure that displays things according to the schema ! разные виды отображения структуры, которые сохраняются в локал стораж. 1. видовс подобный - водновременно открыты 3 панели (предыдущая, текущая, следущая). 2. листающаяся - просто листает горизонтально все карты
...
сделан на объекте client как в призме, то есть можно запрашивать только 1 тип. альтеранатива: запрашивать документ с объекта клиент или при дереференсе значит мб несколько типов, поэтому их надо указать в where, а select будет объектов где ключ это каждый тип у него будут свои запросы.

(внизу звучит как хрень)

```ts
// запрос на типы документов
query
  where
    // выбрать документы где тип shop или user
    type shop | user
  select
    // запрос на тип документа, для кадого такого документа указать его запрос
    shop
      select ...
    user
      where ...
...
если делать запрос только на 1 типе документа

как будто лучше

```ts
// запрос на тип документа
query
  where
    name icludes "..."
  select
    hobby // ссылка на документ типа tennis | coding
      where
      select // потому что документа на hobby может быть нескольких типов, юнион, то селект становится объектом
        teenis
        coding

response
  hobby // type: tennis | type coding
    // was not coding
    tennis
      ...fields
```
