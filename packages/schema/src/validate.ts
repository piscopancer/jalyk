...
const buildPath = (segments: readonly (string | number)[]) =>
  makeFieldPath(segments.map(String))

const makeIssue = (
  opts: IssueOptions | undefined,
  fallback: string,
): Issue => ({
...
const numberInRange = ({ min, max }: { min?: number; max?: number }) => {
  const lower =
    min === undefined
      ? Schema.Number
      : Schema.Number.pipe(Schema.greaterThanOrEqualTo(min))
...
export const rules = {
  ...
  minLength: (value: unknown, n: number, opts?: IssueOptions) =>
    check(
      value,
      Schema.String.pipe(Schema.minLength(n)),
      opts,
      `Не короче ${n} символов`,
    ),
  maxLength: (value: unknown, n: number, opts?: IssueOptions) =>
    check(
      value,
      Schema.String.pipe(Schema.maxLength(n)),
      opts,
      `Не длиннее ${n} символов`,
    ),
  pattern: (value: unknown, re: RegExp, opts?: IssueOptions) =>
    check(
      value,
      Schema.String.pipe(Schema.pattern(re)),
      opts,
      'Неверный формат',
    ),
  minItems: (value: unknown, n: number, opts?: IssueOptions) =>
    check(
      value,
      Schema.Array(Schema.Unknown).pipe(Schema.minItems(n)),
      opts,
      `Не меньше ${n} элементов`,
    ),
  maxItems: (value: unknown, n: number, opts?: IssueOptions) =>
    check(
      value,
      Schema.Array(Schema.Unknown).pipe(Schema.maxItems(n)),
      opts,
  ...
  range: (
    ...
    opts?: IssueOptions,
  ) => check(value, numberInRange(bounds), opts, rangeMessage(bounds)),
  schema: <A>(
    value: unknown,
    schema: Schema.Schema<A, any, never>,
    opts?: IssueOptions,
  ) => check(value, schema, opts, 'Неверное значение'),
}
...
export type DocumentValidate<Draft, F extends FieldMap> = (
  doc: Draft,
  path: PathBuilder<F>,
) => readonly RuleResult[]
