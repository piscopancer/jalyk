...
// рисует либо переданный потребителем вид (children), либо выбранный лейаут.
function StudioBody({ layout = 'miller', children }: { layout?: StudioLayout; children?: ReactNode }) {
  useLiveInvalidation()
...
function StudioShell({ projectId, apiKey, apiUrl, config, fieldComponents, layout, children }: StudioProps) {
  const dark = useStudioDark()
  // Корневой элемент студии становится контейнером для порталов Base UI, чтобы
  // всплывающие слои рендерились внутри `.dark` и наследовали тему и шрифт.
