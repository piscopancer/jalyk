import type { FieldMap, FormComponentProps, FormFieldRenderProps } from '@jalyk/schema'
...
// Кастомная форма редактора (formComponent документа): студия рисует её вместо
// дефолтного перебора полей. Пропсы параметризованы картой полей F, а не всем
// конфигом — иначе конфиг сослался бы на себя через свой же компонент (цикл).

/** Пропсы <Field> со стороны студии: узлы рендера — ReactNode. */
export type FormFieldProps<F extends FieldMap> = FormFieldRenderProps<F, ReactNode>

/** Компонент отрисовки поля по ключу, типизированный картой полей F. */
export type FormFieldComponent<F extends FieldMap> = (props: FormFieldProps<F>) => ReactNode
...
function FormField({ name, header, headerComponent, className }: { name: string; header?: unknown; headerComponent?: unknown; className?: string }) {
  const { config } = useStudio()
  const { type } = useDocumentContext()
  const field = config.documents[type]?.fields[name]
  if (!field) return null
  return <FieldInput path={[name]} field={field} header={header} headerComponent={headerComponent} className={className} />
}

/** Собирает пропсы формы; хэндлы строятся перебором ключей конфига — набор статичен, порядок useField стабилен. */
function useFormProps(fields: Record<string, unknown>): { document: { id: string; type: string; draft: Record<string, unknown> }; Field: typeof FormField; fields: Record<string, FieldHandle<unknown>> } {
  ...
  return {
    document: { id, type, draft: draft !== null && typeof draft === 'object' ? (draft as Record<string, unknown>) : {} },
    Field: FormField,
    fields: handles,
  }
}
...
export function CustomForm({ fields, component: Component }: { fields: Record<string, unknown>; component: (props: FormProps<FieldMap>) => ReactNode }) {
