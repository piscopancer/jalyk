...
// Интерактивный просмотр черновика документа — «наполовину JSON»: объекты и массивы сворачиваются, а ссылка (reference) — это вложенный блок, который при раскрытии лениво подгружает черновик связанного документа (его рисует тот же рекурсивный узел). Только чтение; форму draft диктует клиентская схема.
...
function ReferenceBody({ refId }: { refId: string }) {
  ...
  if (!doc.data)
    return <span className="text-xs text-destructive">Документ недоступен</span>
