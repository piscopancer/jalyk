...
import { Button, Input, Popover, PopoverContent, PopoverTrigger, cn } from '@jalyk/ui'
...
import { compareDocs, compileFilterPredicate, gatherFilterTypes, matchesSearch, type EvalContext } from '../data/list-query.ts'
import { emptyFilter, useFilterCount, useListState, type ListState } from '../data/list-state.ts'
...
// Поля поиска по умолчанию — все строковые поля документа (поиск есть на любом
// типе, даже без list.search в конфиге).
...
export function DocumentRow({ doc, selected, onSelect }: { doc: PreviewDoc; selected?: string; onSelect: (id: string) => void }) {
...
// Состояние «выключенных» элементов управления: используем пустое, не читая и не
// записывая localStorage (для встроенных списков с предустановленным фильтром).
const NO_CONTROLS: ListState = { search: '', filter: emptyFilter(), sort: null }
...
// Переиспользуемый список документов одного типа: панель управления (поиск, древо
// фильтров, сортировка, создание) плюс отфильтрованные/отсортированные строки.
// Поиск, фильтр и сортировка исполняются на клиенте поверх загруженных по типу
// документов (тот же движок, что у типизированного клиента запросов).
export function DocumentList({ type, selected, onSelect, controls = true, where, searchFields }: DocumentListProps) {
  ...
  const fields = searchFields ?? def?.list?.search ?? defaultSearchFields(def?.fields ?? {})

  // Типы связанных документов, на которые ссылается фильтр, — догружаем их для
  // дереференса реляций (some/every/none по полям связанного документа).
  ...
  const relResults = useQueries({
    queries: relTypes.map((relType) => ({
      ...
      queryFn: () => run(client.documents.list({ path: { projectId }, urlParams: { type: relType } })),
    })),
  })
  ...
  const ctx = useMemo<EvalContext>(() => {
    ...
    const put = (docType: string, list: readonly { id: string; draft: unknown }[] | undefined) => {
  ...
  const predicate = useMemo(() => compileFilterPredicate(config.documents, type, state.filter), [config, type, state.filter])

  const rows = (documents.data ?? [])
    .filter((doc) => matchesSearch(doc.draft, fields, state.search))
    .filter((doc) => predicate(doc.draft, ctx))
    .filter((doc) => (where ? matchesWhere({ id: doc.id, draft: doc.draft }, where) : true))
    .sort((a, b) => compareDocs(a, b, state.sort))

  return (
    <div className="flex w-72 shrink-0 flex-col overflow-hidden border-r">
      {controls ? <ListToolbar type={type} searchFields={fields} onCreated={(id) => onSelect(id)} /> : null}
      <ul className="flex flex-col overflow-y-auto">
        {documents.isLoading ? <li className="px-3 py-2 text-xs text-muted-foreground">Загрузка…</li> : null}
        {rows.map((doc) => (
        ...
        {documents.data && rows.length === 0 ? (
          <li className="px-3 py-2 text-xs text-muted-foreground">{documents.data.length === 0 ? 'Документов нет' : 'Ничего не найдено'}</li>
        ) : null}
      </ul>
    </div>
  )
...
function ListToolbar({ type, searchFields, onCreated }: { type: string; searchFields: readonly string[]; onCreated: (id: string) => void }) {
  ...
  return (
    <div className="flex items-center gap-2 border-b p-2">
      ...
      <div className="flex flex-1 items-center">
        {hasSearch ? (
          <div className="relative flex-1">
            ...
            <Input
              value={state.search}
              onChange={(e) => setState((draft) => void (draft.search = e.target.value))}
              placeholder="Поиск"
        ...
        <Popover>
          <PopoverTrigger
            render={<Button size="icon-sm" variant="outline" aria-label="Фильтры" className={cn('relative rounded-none border-l-0', !hasSearch && 'rounded-l-lg border-l')} />}
          >
            <FilterIcon />
            {filterCount > 0 ? (
          ...
          <PopoverContent align="start" className="w-[32rem] max-w-[calc(100vw-2rem)]">
            <FilterBuilder type={type} stateKey={key} />
          </PopoverContent>
        </Popover>

        <SortMenu type={type} stateKey={key} className="rounded-l-none border-l-0" />
      </div>
      ...
      <Button
        ...
        onClick={() => create.mutate({ type }, { onSuccess: (doc) => onCreated(doc.id) })}
      >
        <PlusIcon />
      </Button>
    </div>
