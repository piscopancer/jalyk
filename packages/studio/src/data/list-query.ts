...
// Чистые помощники применения состояния списка к загруженным документам. Скалярные условия исполняются через matchesWhere из @jalyk/schema (тот же движок, что у типизированного клиента); реляционные условия (фильтр по полям связанного документа с квантификатором some/every/none) исполняются здесь же, дореференсивая ссылки по карте уже загруженных документов (ctx.docs).
...
/** Карта типов → поля; берётся из config.documents. Тип нарочно структурный, чтобы не тянуть AnyConfig. */
...
function relatedRefs(
  ...
  const toRef = (value: unknown): { _ref: string; _toType: string } | null =>
    ...
    typeof value._toType === 'string'
      ? { _ref: value._ref, _toType: value._toType }
      : null
  if (multiple)
...
/** Компилирует древо фильтра в предикат над черновиком. Скаляры исполняет matchesWhere; группы — and/or детей; реляции дереференсят ссылки через ctx.docs и применяют квантификатор some/every/none к вложенному фильтру по полям связанного документа. Цели реляций берутся из defs (карта типов). */
export function compileFilterPredicate(
  ...
  if (node.kind === 'group') {
    const preds = node.children.map((child) =>
      compileFilterPredicate(defs, type, child),
    )
    if (preds.length === 0) return ALWAYS
    return (draft, ctx) =>
      node.op === 'or'
        ? preds.some((p) => p(draft, ctx))
        : preds.every((p) => p(draft, ctx))
  }
  // реляция
  const field = defs[type]?.fields?.[node.field]
  const targets = field ? relationTargets(field) : null
  if (node.field === '' || !field || !targets || targets.length === 0)
    return ALWAYS
  ...
  return (draft, ctx) => {
    const refs = relatedRefs(draft, node.field, multiple).filter(
      (ref) => ref._toType === target,
    )
    ...
    if (quantifier === 'every')
      return drafts.every((related) => inner(related, ctx))
    if (quantifier === 'none')
...
export function gatherFilterTypes(
  defs: DocDefs,
  type: string,
  node: FilterNode,
  acc: Set<string>,
