...
/** Точечная запись поля-по-пути. Низкоуровневая мутация; поверх неё строится useField (см. field-хуки). Оптимистично патчит draft в кэше документа по пути сразу при старте мутации, чтобы редактор (особенно перетаскивание элементов массива) не показывал старое значение до ответа сервера; при ошибке откатывает снапшот. По завершении инвалидирует списки документов того же типа (studioKeys.documents(type) — префикс покрывает и список, и запросы query этого типа), чтобы строки списка и их превью с индикаторами замечаний обновились. Тип нужен именно для сужения: префикс documentsAll рефетчил бы и справочники/пикеры чужих типов, открытые в редакторе (например список групп в поле-ссылке альбома), заставляя их поля перерисовываться на каждую правку. */
export function useSetField(id: string, type: string) {
  const { projectId, client, run } = useStudio()
  const qc = useQueryClient()
  return useMutation({
