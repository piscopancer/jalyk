...
// Хэндл поля документа: типизированный доступ к значению по пути плюс точечная
// запись и статус мутации. Подписан на SSE-события того же документа и пути —
// правка из другого клиента патчит кэш и приходит сюда как новое value.
...
/**
 * Доступ к полю текущего документа (из <DocumentProvider>) по пути. Дженерик T —
 * ожидаемый тип значения; вызывающий редактор задаёт его из своей схемы.
 */
export function useField<T = unknown>(path: readonly string[]): FieldHandle<T> {
  ...
  // Удалённые правки этого же поля: патчим кэш документа по пути из события,
  // чтобы value обновилось без перезапроса. Своя правка тоже вернётся сюда —
  // патч до того же значения безвреден.
  const pathKey = JSON.stringify(path)
  useEffect(
    () =>
      subscribeEvents((event) => {
        if (event.kind === 'field' && event.docId === id && samePath(event.path, path)) {
          qc.setQueryData<CachedDocument>(studioKeys.document(projectId, id), (prev) =>
            prev ? { ...prev, draft: setAtPath(prev.draft, event.path, event.value) } : prev,
          )
        }
      }),
