...
// Состояние панели списка документов (поиск, древо фильтров, сортировка) на jotai.
// Персистится в localStorage через atomWithStorage (автосохранение), правки древа
// идут через withImmer (глубокая мутация без ручного копирования), а точечные
// производные значения читаются через selectAtom, чтобы не перерисовывать всё.
...
export type FilterOp = 'contains' | 'startsWith' | 'equals' | 'lt' | 'lte' | 'gt' | 'gte'
...
export type FilterCond = { kind: 'cond'; id: string; field: string; not: boolean; op: FilterOp; value: string | number | boolean }
...
export type FilterRel = { kind: 'rel'; id: string; field: string; target: string; quantifier: RelQuantifier; group: FilterGroup }

/** Узел-группа: объединяет детей через and/or. */
export type FilterGroup = { kind: 'group'; id: string; op: 'and' | 'or'; children: FilterNode[] }
...
export type ListState = { search: string; filter: FilterGroup; sort: SortState }
...
export function newCond(): FilterCond {
  return { kind: 'cond', id: nodeId(), field: '', not: false, op: 'equals', value: '' }
}

// По умолчанию сортируем по дате обновления, новые сверху.
const defaultListState: ListState = { search: '', filter: emptyFilter(), sort: { field: 'updatedAt', dir: 'desc' } }

// Атом на пару проект+тип; ключ — `${projectId}:${type}`. atomFamily кэширует
// инстансы, поэтому одно и то же состояние шарится между панелью и списком.
// Префикс версии — модель древа фильтров менялась (cond/rel-узлы, квантификаторы),
// старое состояние несовместимо, поэтому ключ хранилища версионируется.
const listStateFamily = atomFamily((key: string) => atomWithStorage<ListState>(`jalyk:list:v4:${key}`, defaultListState))
...
export function useFilterCount(key: string): number {
  const base = listStateFamily(key)
  const countAtom = useMemo(() => selectAtom(base, (state) => countLeaves(state.filter)), [base])
